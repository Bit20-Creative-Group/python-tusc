...
class Dex(BlockchainInstance):
    ...
    .. note:: The methods of this class only deal with a single asset (at
              ...
              please use :class:`tusc.aio.market.Market`.
    """
    ...
    async def returnFees(self):
        ...
        """
        from tuscbase.operations import operations

        r = {}
        obj, base = await self.blockchain.rpc.get_objects(["2.0.0", "1.3.0"])
    ...
    async def close_debt_position(self, symbol, account=None):
        ...
        return await self.blockchain.finalizeOp(op, account["name"], "active")
    ...
    async def adjust_debt(
        ...
        current_debts = await self.list_debt_positions(account)
        ...
        collateral_asset = await Asset(
            backing_asset_id, blockchain_instance=self.blockchain
        )
        settlement_price = await Price(
            bitasset["current_feed"]["settlement_price"],
            blockchain_instance=self.blockchain,
