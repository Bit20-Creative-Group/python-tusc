...
class Market(SyncMarket):
    ...
    * ``base`` and ``quote`` are valid assets (according to :class:`tusc.asset.Asset`)
    ...
    * ``base-quote`` separated with ``-``
    ...
    .. note:: Throughout this library, the ``quote`` symbol will be
              ...
              :func:`tusc.market.Market.buy`, you will sell/buy **only
    ...
    async def ticker(self):
        ...
        * ``price24h``: the price 24h ago
        ...
        if "bitasset_data_id" in self["quote"]:
            bitasset = await self.blockchain.rpc.get_object(
                self["quote"]["bitasset_data_id"]
            )
            ...
            if backing_asset_id == self["base"]["id"]:
                ...
                data["quoteSettlement_price"] = await Price(
                    sp, blockchain_instance=self.blockchain
                )
                if sp["base"]["asset_id"] == self["quote"]["id"]:
                    data["quoteSettlement_price"] = await data[
                        "quoteSettlement_price"
                    ].invert()

        elif "bitasset_data_id" in self["base"]:
            bitasset = await self.blockchain.rpc.get_object(
                self["base"]["bitasset_data_id"]
            )
            backing_asset_id = bitasset["options"]["short_backing_asset"]
            if backing_asset_id == self["quote"]["id"]:
                data["baseSettlement_price"] = await Price(
                    bitasset["current_feed"]["settlement_price"],
                    blockchain_instance=self.blockchain,
                )

        ticker = await self.blockchain.rpc.get_ticker(
            self["base"]["id"], self["quote"]["id"]
        )
        data["baseVolume"] = await Amount(
            ticker["base_volume"] or 0.0,
            self["base"],
            blockchain_instance=self.blockchain,
        )
        data["quoteVolume"] = await Amount(
            ticker["quote_volume"] or 0.0,
            self["quote"],
            blockchain_instance=self.blockchain,
        )
        data["lowestAsk"] = await Price(
            ticker["lowest_ask"] or 0.0,
            base=self["base"],
            quote=self["quote"],
            blockchain_instance=self.blockchain,
        )
        data["highestBid"] = await Price(
            ticker["highest_bid"] or 0.0,
            base=self["base"],
            quote=self["quote"],
            blockchain_instance=self.blockchain,
        )
        data["latest"] = await Price(
    ...
    async def volume24h(self):
        ...
        """
        volume = await self.blockchain.rpc.get_24_volume(
            self["base"]["id"], self["quote"]["id"]
        )
        return {
            self["base"]["symbol"]: await Amount(
                volume["base_volume"], self["base"], blockchain_instance=self.blockchain
            ),
            self["quote"]["symbol"]: await Amount(
                volume["quote_volume"],
                self["quote"],
                blockchain_instance=self.blockchain,
            ),
    ...
    async def orderbook(self, limit=25):
        """
    ...
    async def trades(self, limit=25, start=None, stop=None):
        ...
        while True:
            ...
            if not sequence:
                ...
                orders = await self.blockchain.rpc.get_trade_history(
                    self["base"]["symbol"],
                    self["quote"]["symbol"],
                    formatTime(stop),
                    formatTime(start),
                ...
                orders = await self.blockchain.rpc.get_trade_history_by_sequence(
                    self["base"]["symbol"],
                    self["quote"]["symbol"],
                    sequence,
                    formatTime(start),
    ...
    async def accounttrades(self, account=None, limit=25):
        """
        ...
        filled = await self.blockchain.rpc.get_fill_order_history(
    ...
    async def accountopenorders(self, account=None):
        """
        Returns open Orders.
        ...
        for o in orders:
            if (
                ...
                r.append(await Order(o, blockchain_instance=self.blockchain))
        return r
    ...
    async def buy(
        ...
        **Example:** in the USD_TUSC market, a price of 300 means
        ...
        if isinstance(price, Price):
            price = await price.as_base(self["base"]["symbol"])
        ...
        tx = await self.blockchain.finalizeOp(
            order, account["name"], "active", **kwargs
        )
    ...
    async def cancel(self, orderNumber, account=None, **kwargs):
        """
        ...
        """
        return await self.blockchain.cancel(orderNumber, account=account, **kwargs)

    async def core_quote_market(self):
        """
        ...
        return await Market(quote=self["quote"], base=collateral)

    async def core_base_market(self):
        """
        ...
        return await Market(quote=self["base"], base=collateral)
