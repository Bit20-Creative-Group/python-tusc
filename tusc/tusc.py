...
from tuscapi.tuscnoderpc import TUSCNodeRPC
from tuscbase import operations
from tuscbase.account import PublicKey
from tuscbase.asset_permissions import asset_permissions, toint
...
class TUSC(AbstractGrapheneChain):
    """
    Connect to the TUSC network.
    ...
    * **Wallet Database**: Here, the tusclibs load the keys from the
      locally stored wallet SQLite database (see ``storage.py``).
      To use this mode, simply call ``TUSC()`` without the
      ``keys`` parameter
    * **Providing Keys**: Here, you can provide the keys for
      your accounts manually. All you need to do is add the wif
      keys for the accounts you want to use as a simple array
      using the ``keys`` parameter to ``TUSC()``.
    * **Force keys**: This more is for advanced users and
    ...
    def create_account(
        ...
        **kwargs
        ...
        from tuscbase.account import PasswordKey, PublicKey
    ...
    def publish_price_feed(
        ...
        ), "settlement_price needs to be instance of `tusc.price.Price`!"
        assert cer is None or isinstance(
            cer, Price
        ), "cer needs to be instance of `tusc.price.Price`!"
        if not account:
            if "default_account" in self.config:
                account = self.config["default_account"]
        if not account:
    ...
    def create_worker(
        ...
        **Optional**
        ...
        :param str account: (optional) the account to allow access
            to (defaults to ``default_account``)
        """
        from tuscbase.transactions import timeformat
